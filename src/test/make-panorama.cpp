...
void do_doruk1(void)
{
  string indir="pano/doruk1/";
  string outdir="output/doruk1/";
  
  image_map im;
  load_images(im,indir,outdir,8,1,2750);  // im, dir, numpics (0..numpics-1),  PROJ_METHOD,  FOCAL_LEN
  
  vector < thread > th; 
  
  th.push_back(thread([&](){create_panorama(im,"0-1","0","1"     ,2,0,0.1,7,7,5,50000,100,0.5);}));
  th.push_back(thread([&](){create_panorama(im,"2-3","2","3"     ,2,0,0.1,7,7,5,50000,100,0.5);}));
  th.push_back(thread([&](){create_panorama(im,"4-5","4","5"     ,2,0,0.1,7,7,5,50000,100,0.5);}));
  th.push_back(thread([&](){create_panorama(im,"6-7","6","7"     ,2,0,0.1,7,7,5,50000,100,0.5);}));
  for(auto&e1:th)e1.join();th.clear();
  
  
  th.push_back(thread([&](){create_panorama(im,"0--3" ,  "2-3", "0-1"    ,2,0,0.1,11,7,5,50000,100,0.5);}));
  th.push_back(thread([&](){create_panorama(im,"4--7" ,  "6-7",  "4-5"   ,2,0,0.1,11,7,5,50000,100,0.5);}));
  for(auto&e1:th)e1.join();th.clear();
  
  create_panorama(im,"all" ,  "4--7", "0--3"    ,2,0,0.1,11,7,5,50000,100,0.5);
...
void do_doruk2(void)
{
  string indir="pano/doruk2/";
  string outdir="output/doruk2/";
  
  image_map im;
  load_images(im,indir,outdir,7,0,50);  // im, dir, numpics (0..numpics-1),  PROJ_METHOD,  FOCAL_LEN
  ...
  th.push_back(thread([&](){create_panorama(im,"0--3" ,  "2-3", "0-1"    ,2,0,0.04,11,7,5,50000,100,0.5);}));
  th.push_back(thread([&](){create_panorama(im,"4--6" ,  "6",   "4-5"    ,2,0,0.04,11,7,5,50000,100,0.5);}));
  for(auto&e1:th)e1.join();th.clear();

  //create_panorama(im,"0--15" , "8--15", "0--7" ,2,0,0.04,11,7,5,50000,100,0.5);
  create_panorama(im,"0--6" ,  "4--6", "0--3"    ,2,0,0.04,11,7,5,50000,100,0.5);
...
void do_doruk3(void)
{
  string indir="pano/doruk3/";
  string outdir="output/doruk3/";
  
  image_map im;
  load_images(im,indir,outdir,11,2,2750);  // im, dir, numpics (0..numpics-1),  PROJ_METHOD,  FOCAL_LEN
  ...
  th.push_back(thread([&](){create_panorama(im,"0--3" ,  "2-3", "0-1"    ,2,0,0.04,11,7,5,50000,100,0.5);}));
  th.push_back(thread([&](){create_panorama(im,"4--7" ,  "6-7",   "4-5"    ,2,0,0.04,11,7,5,50000,100,0.5);}));
  th.push_back(thread([&](){create_panorama(im,"8--10" , "8-9",   "10"    ,2,0,0.04,11,7,5,50000,100,0.5);}));
  for(auto&e1:th)e1.join();th.clear();

  create_panorama(im,"0--7" , "0--3", "4--7" ,2,0,0.04,11,7,5,50000,100,0.5);
  create_panorama(im,"all" ,  "0--7", "8--10"    ,2,0,0.04,11,7,5,50000,100,0.5);
...
void do_doruk4(void)
{
  string indir="pano/doruk4/";
  string outdir="output/doruk4/";
  
  image_map im;
  load_images(im,indir,outdir,5,0,50);  // im, dir, numpics (0..numpics-1),  PROJ_METHOD,  FOCAL_LEN
  ...
  //create_panorama(im,"0--15" , "8--15", "0--7" ,2,0,0.04,11,7,5,50000,100,0.5);
  create_panorama(im,"0--4" ,  "0--3", "4"    ,2,0,0.04,11,7,5,50000,100,0.5);
...
void do_doruk5(void)
{
  string indir="pano/doruk5/";
  string outdir="output/doruk5/";
  
  image_map im;
  load_images(im,indir,outdir,15,1,2250);  // im, dir, numpics (0..numpics-1),  PROJ_METHOD,  FOCAL_LEN
  
  vector < thread > th; 
  
  th.push_back(thread([&](){create_panorama(im,"0-1","0","1"     ,2,0,0.03,7,7,5,50000,100,0);}));
  th.push_back(thread([&](){create_panorama(im,"2-3","2","3"     ,2,0,0.03,7,7,5,50000,100,0);}));
  th.push_back(thread([&](){create_panorama(im,"4-5","4","5"     ,2,0,0.03,7,7,5,50000,100,0);}));
  th.push_back(thread([&](){create_panorama(im,"6-7","6","7"     ,2,0,0.03,7,7,5,50000,100,0);}));
  th.push_back(thread([&](){create_panorama(im,"8-9","8","9"     ,2,0,0.03,7,7,5,50000,100,0);}));
  th.push_back(thread([&](){create_panorama(im,"10-11","10","11"     ,2,0,0.03,7,7,5,50000,100,0);}));
  th.push_back(thread([&](){create_panorama(im,"12-13","12","13"     ,2,0,0.03,7,7,5,50000,100,0);}));
  for(auto&e1:th)e1.join();th.clear();

  th.push_back(thread([&](){create_panorama(im,"0--3" ,  "2-3", "0-1"    ,2,0,0.03,11,7,5,50000,100,0);}));
  th.push_back(thread([&](){create_panorama(im,"4--7" ,  "6-7",   "4-5"    ,2,0,0.03,11,7,5,50000,100,0);}));
  th.push_back(thread([&](){create_panorama(im,"8--11" ,  "8-9",   "10-11"    ,2,0,0.03,11,7,5,50000,100,0);}));
  th.push_back(thread([&](){create_panorama(im,"12--14" ,  "12-13",   "14"    ,2,0,0.03,11,7,5,50000,100,0);}));
  for(auto&e1:th)e1.join();th.clear();

  th.push_back(thread([&](){create_panorama(im,"0--7" ,  "4--7", "0--3"    ,2,0,0.03,11,7,5,50000,100,0);}));
  th.push_back(thread([&](){create_panorama(im,"8--14" ,  "12--14", "8--11"    ,2,0,0.03,11,7,5,50000,100,0);}));
  for(auto&e1:th)e1.join();th.clear();

  create_panorama(im,"all" ,  "8--14", "0--7"    ,2,0,0.03,11,7,5,50000,100,0.5);
...
void do_doruk6(void)
{
  string indir="pano/doruk6/";
  string outdir="output/doruk6/";
  ...
  create_panorama(im,"all" ,  "4--7", "0--3"    ,2,0,0.04,11,7,5,50000,100,0.5);
...
int main(int argc, char **argv)
{
  ...
  if(string(argv[1])=="doruk1")do_doruk1();
  if(string(argv[1])=="doruk2")do_doruk2();
  if(string(argv[1])=="doruk3")do_doruk3();
  if(string(argv[1])=="doruk4")do_doruk4();
  if(string(argv[1])=="doruk5")do_doruk5();
  if(string(argv[1])=="doruk6")do_doruk6();
  return 0;
}
