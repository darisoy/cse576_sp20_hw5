...
{ 
  ...
  for (int x = 0; x < dimension; x++) 
  {
    ...
    float exponent = -1.0 * pow(newx, 2) / (2*pow(sigma, 2));
...
{
  // use two convolutions with 1d gaussian filter.
...
Image structure_matrix(const Image& im2, float sigma)
{
  ...
  Matrix IxIx = elementwise_multiply(Ix, Ix);
  Matrix IyIy = elementwise_multiply(Iy, Iy);
  Matrix IxIy = elementwise_multiply(Ix, Iy);
  for (int w = 0; w < im.w; w++)
      for (int h = 0; h < im.h; h++) 
      {
        S(w, h, 0) = IxIx(w, h);
...
Image cornerness_response(const Image& S, int method)
{
  ...
  for (int w = 0; w < S.w; w++)
    for (int h = 0; h < S.h; h++) 
    {
      ...
      float IxIy = S(w, h, 2);
      if (method == 0)
        R(w, h) = ((IxIx*IyIy) - pow(IxIy,2)) / (IxIx+IyIy);
      else if (method == 1)
        R(w, h) = (IxIx*IyIy) - pow(IxIy,2) - (0.06 * pow(IxIx+IyIy,2));
    }
  return R;
}
...
Image nms_image(const Image& im, int w)
{
  ...
  for (int x = 0; x < im.w; x++)
    for (int y = 0; y < im.h; y++)
      for (int neigh_x = x-w; neigh_x <= x+w; neigh_x++)
        for (int neigh_y = y-w; neigh_y <= y+w; neigh_y++)
          if (im(x, y, 0) < im.clamped_pixel(neigh_x, neigh_y, 0))
            r(x, y, 0) = -INFINITY;
...
{
  vector<Descriptor> d;
  //count number of responses over threshold (corners)
  //and fill in vector<Descriptor> with descriptors of corners, use describe_index.
